...
class WebsiteService:
    ...
    async def move_website_to_space(
        ...
        if not source_space_actor.can_delete_website(website=website):
            raise UnauthorizedException(
    ...
    async def publish_website(self, id: UUID, published: bool):
        website = await self.get_website(id=id)
        space = await self.space_service.get_space(id=website.space_id)
        actor = self.actor_manager.get_space_actor_from_space(space=space)

        if not actor.can_publish_websites():
            raise UnauthorizedException()

        website_update = WebsiteUpdate(id=website.id, published=published)
        return await self.repo.update(website=website_update)
