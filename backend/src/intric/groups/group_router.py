...
from intric.groups import group_protocol
...
async def upload_file(
    ...
    return await group_service.add_file_to_group(
        id=id, file=file.file, mimetype=file.content_type, filename=file.filename
    )
...
async def publish_group(
    ...
    group_published = await service.publish_group(id=id, published=published)
    count = await service.get_count_for_group(group_published)

    return group_protocol.to_group_public_with_metadata(group_published, count)
