...
from intric.spaces.space import UNAUTHORIZED_EXCEPTION_MESSAGE, Space, SpaceRole
...
def test_get_default_model(space: Space):
    gpt_4o_model = MagicMock()
    gpt_4o_model.name = "gpt-4o"
    gpt_4o_model.can_access = True
    space.completion_models = [gpt_4o_model]
    assert space.get_default_model() is not None

    # Disable access
    gpt_4o_model.can_access = False
    assert space.get_default_model() is None

    claude_model = MagicMock()
    claude_model.name = "claude-3-sonnet-20240229"
    claude_model.can_access = True
    space.completion_models = [claude_model]
    assert space.get_default_model() is None
