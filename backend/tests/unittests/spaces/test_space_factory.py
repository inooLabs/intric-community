...
def test_create_space_with_assistants_and_default_assistant(factory: SpaceFactory):
    ...
    space_in_db = MagicMock(assistants=[normal_assistant, default_assistant])

    # Run
    space = factory.create_space_from_db(
        space_in_db=space_in_db, default_assistant=default_assistant
    )

    # Assert
    assert space.assistants == [AssistantSparse.model_validate(normal_assistant)]
