...
async def test_ask_question_not_in_space(service: AnalysisService):
    service.assistant_service.get_assistant.return_value = AsyncMock(
        space_id=None, user=service.user
    )
...
async def test_ask_question_personal_space_no_access(service: AnalysisService):
    ...
    service.assistant_service.get_assistant.return_value = MagicMock(
        space_id=uuid4(), user=service.user
    )
